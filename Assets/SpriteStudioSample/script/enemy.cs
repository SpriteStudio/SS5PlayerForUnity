...
//		POSE2,
	...
	//キャラクター制御
	// 速度
	private Vector2 SPEED = new Vector2(8.0f, 8.0f);			//移動速度
	...
	int[] cpu_input_key = new int[(int)gamemain.INPUTBUTTON.BUTTON_MAX] ;					//キー入力
	int cpu_nextmove_time = 0;				//次のコマンド入力までの時間
	int cpu_nextjump_wait = 0;				//次のジャンプまでの時間
...
//			case AnimationType.KICK2:
	...
	void charinit()
	{
		...
		player_pos = new Vector2(x, 300);	//プレイヤーの位置（表示位置とは別）
...
//		hit_muteki = 1;		//連続ヒット防止
	...
	{
		int i;
		for ( i = 0 ; i < (int)gamemain.INPUTBUTTON.BUTTON_MAX; i++)
		{
			cpu_input_key[i] = 0;
		}

		//ターゲットまでの距離をから行動を判断する
		Vector2 targetpos = playercntrol.player_pos;
		cpu_renge = fabs( player_pos.x - targetpos.x ); 
		dash = false;

		float jump_renge = 900;
		float dash_renge = 700;
		float run_renge = 300;
		float atk_renge = 350;
		if ( player_pos.x < targetpos.x )
		{
			...
			if ( cpu_renge > run_renge )
			{
				cpu_input_key[(int)gamemain.INPUTBUTTON.BUTTON_RIGHT] = 1;
			}
		}
		else
		{
			...
			if ( cpu_renge > run_renge )
			{
				cpu_input_key[(int)gamemain.INPUTBUTTON.BUTTON_LEFT] = 1;
			}
		}
		if ( ( cpu_renge < atk_renge ) && ( cpu_nextmove_time == 0 ))
		{
			cpu_input_key[(int)gamemain.INPUTBUTTON.BUTTON_1] = 1;
			cpu_nextmove_time = 60;
		}
		cpu_nextmove_time--;
		if ( cpu_nextmove_time < 0)
		{
			cpu_nextmove_time = 0;
		}
		if ( ( cpu_renge < jump_renge ) && ( cpu_nextjump_wait == 0 ))
		{
			if ( UnityEngine.Random.Range (0, 100 ) < 50 )
			{
				...
				cpu_input_key[(int)gamemain.INPUTBUTTON.BUTTON_UP] = 1;
			}
			cpu_nextjump_wait = 60;
		}
		cpu_nextjump_wait--;
		if (cpu_nextjump_wait < 0)
		{
			cpu_nextjump_wait = 0;
		}
